...
def get_data(date):
    ...
    f = urllib.request.urlopen("https://weather.eos.ubc.ca/wxfcst/current/wxapp-achan/" + dateString + "00").read().decode("utf-8")
    arr = f.split('\n\n\n')
    arr[0] = arr[0].split("\n",3)[3]
    arr[1] = arr[1].split("\n",1)[1]
    arr[2] = arr[2].split("\n",2)
...
def get_forecast_frame(arr):
    f = open("weatherupdater/forecast_headers.txt", "r")
    ...
    print(forecastFrame)
    forecastFrame = forecastFrame.set_index('Hour', drop=True)
...
def get_previous_conditions(arr, date):
    ...
    conditionsFrame.to_csv('out.csv')
